...
export default function TestForm(props) {
  ...
  const [result, setResult] = useState({ calculateHours: 0, getWorkingHours: { hours: 0, minutes: 0 }, considerWorkingHours: null });
  ...
  useEffect(() => {
    ...
    console.log("calculateHours", calculateHours, data)
  ...
  return (
    <div className='secondBox'>
      ...
      {result?.calculateHours !== 0 && <>
        ...
        <p className={ `${ result?.calculateHours < 0 ? 'text-success': 'text-danger' }` }>{`${result?.calculateHours < 0 ? 'Lagging Behind' : 'Ahead By'} => ${result?.getWorkingHours?.hours} Hours : ${result?.getWorkingHours?.minutes} minutes`}</p>
      </>
      }
      {
        result &&
          ...
          <table className={`table table-hover ${props.mode === 'light' ? '' : 'table-dark'}`}>
            ...
            <tbody>
              {result.considerWorkingHours.map((workingHours, i) => (
                <tr key={`working-hours-${i}`} className={ workingHours?.compareTimes()?.status === "ahead" ? "table-success": "table-danger" }>
                  <th scope="row">{i + 1}</th>
                  <td>{workingHours}</td>
                  <td>8 hrs, 0 min</td>
